...
const coachContactRows = parseCsv(
  fs.readFileSync(
    path.join(root, 'data', 'lewisville-coach-contacts.csv'),
    'utf8',
  ),
);
const coachContactHeader = coachContactRows.shift() || [];
if (coachContactHeader.join('|') !== coachContactColumns.join('|'))
  throw new Error(
    `lewisville-coach-contacts.csv has unexpected columns. Expected: ${coachContactColumns.join(', ')}`,
  );
const coachContacts = coachContactRows.map((row, rowIndex) => {
  if (row.length !== coachContactHeader.length)
    throw new Error(
      `lewisville-coach-contacts.csv row ${rowIndex + 2} has ${row.length} cells; expected ${coachContactHeader.length}.`,
    );
  return Object.fromEntries(
    coachContactHeader.map((column, columnIndex) => [
      column,
      row[columnIndex].trim(),
    ]),
  );
});
const normalizeCollegeName = (school) =>
  school
    .toLowerCase()
    .replaceAll('&', ' and ')
    ...
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
const collegeCoachDirectory = {};
coachContacts
  .filter((coach) => coach.x_url)
  .forEach((coach) => {
    const key = normalizeCollegeName(coach.school);
    if (!collegeCoachDirectory[key]) collegeCoachDirectory[key] = [];
    collegeCoachDirectory[key].push([
      coach.full_name,
      coach.position,
      coach.x_url,
    ]);
...
const coachDirectoryOutput = path.join(
  root,
  'static',
  'data',
  'college-coach-x.json',
);
const coachDirectoryJson = `${JSON.stringify(collegeCoachDirectory)}\n`;
...
if (checking) {
  ...
  if (
    !fs.existsSync(coachDirectoryOutput) ||
    fs.readFileSync(coachDirectoryOutput, 'utf8') !== coachDirectoryJson
  )
    throw new Error(
      'Generated college coach directory is stale. Run npm run data:sync.',
    );
  console.log(
  ...
  fs.writeFileSync(coachDirectoryOutput, coachDirectoryJson);
