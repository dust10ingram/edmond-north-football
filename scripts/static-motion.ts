...
if(!reduced){
  ...
  const revealGroups=(selector:string,children:string,amount=.12,duration=1.05,step=.09)=>document.querySelectorAll<HTMLElement>(selector).forEach(section=>{const items=[...section.querySelectorAll<HTMLElement>(children)];if(!items.length)return;prepare(items);inView(section,()=>animate(items,{opacity:1,transform:'translateY(0px) scale(1)'},{duration,delay:stagger(step),ease,onComplete:()=>finish(items)}),{amount})});
  const revealEach=(selector:string,amount=.25,duration=1.2,distance=28)=>document.querySelectorAll<HTMLElement>(selector).forEach(item=>{prepare([item],distance);inView(item,()=>animate(item,{opacity:1,transform:'translateY(0px) scale(1)'},{duration,ease,onComplete:()=>finish([item])}),{amount})});

  revealEach(['.section-lead h2','.program-copy h2','.media-copy h2','.coaches > h2','.support-copy h2','.sponsors-copy h2','.standalone-schedule > h1','.level-schedule-heading h2','.recruiting-intro h1','.coach-finder-copy h2','.roster-hero h1','.camps-hero h1','.camp-list h2','.donate-hero h1','.donate-story h2','.donation-heading h2','.huskies-hero h1','.huskies-history h2','.all-time h2','.alumni h2','.menu-page section > h1','.gameday-hero h1','.program-story h2','.weekly-board-heading h2','.program-sponsors h2','.program-update h2'].join(','),.22,1.25,30);

  const tradition=document.querySelector<HTMLElement>('.tradition');
  if(tradition){const eyebrow=tradition.querySelector<HTMLElement>(':scope > .eyebrow');const quote=tradition.querySelector<HTMLElement>(':scope > blockquote');if(eyebrow)prepare([eyebrow],14);if(quote)prepare([quote],34,.985);if(quote)inView(quote,()=>{if(eyebrow)animate(eyebrow,{opacity:1,transform:'translateY(0px) scale(1)'},{duration:1.05,ease,onComplete:()=>finish([eyebrow])});animate(quote,{opacity:1,transform:'translateY(0px) scale(1)'},{duration:1.65,delay:.18,ease:slowEase,onComplete:()=>finish([quote])})},{amount:.25});revealEach('.tradition > .roster-cta,.tradition .program-levels article',.2,1.1,26)}

  revealGroups('.standalone-schedule','.schedule-list article',.08,1.05,.07);
  revealGroups('.recruiting-standalone','.player-profile',.04,1.05,.06);
  revealGroups('.weekly-board','.leader-grid article',.12,1.05,.08);

  document.querySelectorAll<HTMLElement>('.upcoming-logo-crop').forEach(mark=>{prepare([mark],18,.97);inView(mark,()=>animate(mark,{opacity:1,transform:'translateY(0px) scale(1)'},{duration:1.05,ease,onComplete:()=>finish([mark])}),{amount:.45})});
