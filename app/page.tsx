...
export default function Home() {
  return <main>
    ...
    <section id="schedule" className="schedule-strip"><div className="section-lead"><div><p className="eyebrow blue">Next up</p><h2>Friday nights start here.</h2></div><a href="/schedule/">Full schedule <span aria-hidden="true">→</span></a></div><div className="game-row">{games.slice(0,3).map((game,index)=><article className={index===0?'game featured':'game'} key={game.opponent}><div className="game-date">{game.date}</div><img src={`/assets/opponents/${game.logo}`} alt=""/><div><p>{game.opponent}</p><span>{game.detail}</span></div><div className="game-place"><span aria-hidden="true">◆</span>{game.place}</div></article>)}</div></section>
