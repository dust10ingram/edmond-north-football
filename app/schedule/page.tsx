const varsityGames = [
  ...
  ['AUG 28','Edmond Santa Fe','Season opener · 7:00 PM','Home','edmond-santa-fe-6c4a73a2.png',''],
  ['SEP 04','Deer Creek','Friday · 7:00 PM','Away','deer-creek-d9a03f17.png',''],
  ['SEP 11','Putnam City North','Friday · 7:00 PM','Away','putnam-city-north-c27f1bef.png',''],
...
export default function Schedule(){return <main className="standalone-page"><header className="subpage-header"><a href="/">← Home</a><img src="/assets/wordmark.png" alt="Edmond North Football"/></header><nav className="schedule-level-nav" aria-label="Team schedules"><a href="#varsity">Varsity</a><a href="#jv">JV</a><a href="#freshman">Freshman</a></nav><section id="varsity" className="full-schedule standalone-schedule"><p className="eyebrow blue">2026 Varsity</p><h1>The road ahead.</h1><div className="schedule-list">{varsityGames.map(([date,opponent,detail,place,logo,ticket],i)=><article key={opponent}><span className="game-number">{String(i+1).padStart(2,'0')}</span><time>{date}</time><img src={`/assets/opponents/${logo}`} alt=""/><div><strong>{opponent}</strong><small>{detail}</small></div><span className={`location ${place.toLowerCase()}`}>{place}</span>{ticket&&<a className="ticket-link" href={ticket} target="_blank" rel="noreferrer">Tickets ↗</a>}</article>)}</div></section><LevelSchedule id="jv" title="JV schedule" games={jvGames}/><LevelSchedule id="freshman" title="Freshman schedule" games={freshmanGames}/></main>}
